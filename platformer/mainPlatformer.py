...
class Game:
    ...
    def run(self,level):
        ...
        while running:
            level.printLevel(self.DISPLAYSURFcopy,self.screenProgress)#A optimiser pour pas faire la bouble a chaque fois, copie sur un frame
    ...
    def events(self):
        ...
        if keyboardState[K_d]:
            self.hero.vx+=80
        elif keyboardState[K_a]:
            self.hero.vx-=80
        else:
    ...
    def printFinalScreen(self):
        ...
        self.DISPLAYSURF.blit( pg.transform.scale(self.DISPLAYSURFcopy,(self.finalWidth,self.finalHeight)) ,(0,0))#???
...
class Hero:
    ...
    def contactTete(self,block):
        ...
        return (block.y<self.get_top()<block.y+st.TILESIZE) and ( (block.x<self.get_left()<block.x+st.TILESIZE) or (block.x<self.get_right()<block.x+st.TILESIZE))
    def contactPieds(self,block):
        return (block.y<self.get_bottom()<block.y+st.TILESIZE) and ( (block.x<self.get_left()<block.x+st.TILESIZE) or (block.x<self.get_right()<block.x+st.TILESIZE) )

    def contactGauche(self,block):
        return (block.x<self.get_left()<block.x+st.TILESIZE) and ( (block.y<self.get_top()<block.y+st.TILESIZE) or (block.y<self.get_bottom()<block.y+st.TILESIZE) or ((block.y<0.5*(self.get_bottom()+self.get_top())<block.y+st.TILESIZE)))
    def contactDroite(self,block):
        return (block.x<self.get_right()<block.x+st.TILESIZE) and ( (block.y<self.get_top()<block.y+st.TILESIZE) or (block.y<self.get_bottom()<block.y+st.TILESIZE) or ((block.y<0.5*(self.get_bottom()+self.get_top())<block.y+st.TILESIZE)))
    #Getters
    def get_top(self):
        return self.y
    def get_bottom(self):
